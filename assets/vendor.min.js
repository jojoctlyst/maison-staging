...
function swatchVariantHandler(_parent, variant, callback) {
  var WCP_JSON=[];
...
function swatchVariantButtonHandler(_parent, variant, callback) {

	var _inputVariant 			= $('.input_variant').val();
  var _btnaddtocart 			= _parent.find('.btn-addtocart');
  var _inputContainer 		= _parent.find('.tt-input-container');
  var _inputCounter 			= _parent.find('.tt-input-counter');
  var _swatches 					= $('.tt-swatches-wrapper').find('.tt-wrapper');
  var swatchesWarning 		= $('<span>').addClass('swatches-warning').text('Pilih size untuk menambahkan produk ke keranjang.');
  var swatchesItems				= _swatches.find('li[data-availability="false"] a');
  var swatchesOpName			= _swatches.find('.tt-title-options').html('Pilih Size: <span class="current_option_name">'+ (variant.option1 ? variant.option1 : '') +'</span><span class="current_option_text"></span>')
  var inventory_quantity 	= String(global_quantity[variant.id]);
  var btn_type 						= $('.input_variant').attr('data-type');

  // size title
  // $('.tt-swatches-wrapper .tt-title-options').replace('')
  ...
  // add disable on swatches items list
 	swatchesItems.each(function( index ) {
	  // $( this ).addClass('disable');
	});

 	if (inventory_quantity == 1 && btn_type != 'shopwa') {
 		swatchesOpName.find('.current_option_text').text('(Sisa 1)').addClass('current_option_text_warning');
 	} else if (inventory_quantity == 0 && btn_type != 'shopwa') {
		swatchesOpName.find('.current_option_text').text('(Stok Habis)');
 	}

	//console.log('swatchVariantButtonHandler', variant.available, variant, String(global_quantity[variant.id]));
	
	if (variant == false && _inputVariant == '') {

		$('.tt-input-counter').addClass('counter-choosevariant-js');
		_btnaddtocart.html('<span>+ Keranjang</span>');
    _btnaddtocart.addClass('btn-choosevariant-js');
    _btnaddtocart.removeClass('disable');
    //_inputCounter.addClass('hide');
    _swatches.append(swatchesWarning);

  } else if(Boolean(variant.available)) {

    var inventory_management = variant.inventory_management;
    var iq = Number(inventory_quantity.split('continue').pop());
    var type = _btnaddtocart.data("type");
		var cvWrap = $('.tt-swatches-wrapper').removeClass('active');

		if (_btnaddtocart.hasClass('btn-choosevariant-js')) {
			_btnaddtocart.removeClass('btn-choosevariant-js');
			$('.tt-input-counter').removeClass('counter-choosevariant-js');
		}
    ...
    if(inventory_management == null){
      ...
      if(iq <= 0){
        ...
        if (type === 'addtocart') {
          _btnaddtocart.html('<span>+ Keranjang</span>');
          _inputContainer.removeClass('hide');
          _inputCounter.removeClass('hide');
        }
        if (type === 'preorder') {
          _btnaddtocart.html('<span>Pre-order by WhatsApp</span>');
        }
        if (type === 'shopwa') {
          _btnaddtocart.html('<span>Shop by WhatsApp</span>');
        }
      }
    }
    ...
    _parent.find('.buyinoneclick').length && _parent.find('.buyinoneclick').removeClass('hidesoldout');
  	...
  	$('.tt-swatches-wrapper').removeClass('active');
    _btnaddtocart.html(unavailable_text).addClass('disable');
    _inputContainer.addClass('hide');
    _inputCounter.addClass('hide');
...
var addToCartHandler = {
  ...
  initFormAddToCartButton: function(variant_id, _parent, _swatch){
    ...
    var _btnPlusMinus = $('.counter-choosevariant-js span');
    ...
    var cvBtnSticky = '';
    var cvWrap = $('.tt-swatches-wrapper');
    var shakeAdd = 0;
    var shakeRemove = 200;

    console.log('Btn Ready', _btn.length);
    
    _btn.on('click', function(e){
      e.preventDefault();
      var cvBtn = $(this).hasClass('btn-choosevariant-js');
      cvBtnSticky = jQuery(".tt-fixed-product-wrapper").is(":visible");
    ...
    _btnPlusMinus.on('click', function(e){
      e.preventDefault();
      var _targ 				= $(this).parent();
      var disable  			= _targ.hasClass('counter-choosevariant-disable');
      var firstVariant 	= _targ.hasClass('counter-choosevariant-js');

      if (!disable) {
	      if (firstVariant) {
	      	cvBtnSticky = jQuery('.tt-fixed-product-wrapper').is(':visible');

	      	_targ.addClass('counter-choosevariant-disable');
	      	
	      	setTimeout(() => {
			    	_targ.removeClass('counter-choosevariant-disable');
			  	}, 1000);
    ...
    function shakeSwaches() {
    	// desktop target
    	var scrollTarget = '.tt-header-holder';
    	if (cvBtnSticky) {

    		if($(window).width() < 576) {
    			scrollTarget = '.tt-mobile-product-slider';
    		} else if($(window).width() < 1025) {
    			scrollTarget = '.tt-product-single-info';
    		}
			...
			setTimeout(() => {
		  	cvWrap.addClass('active').addClass('shake');
		  	setTimeout(() => {
		    	cvWrap.removeClass('shake');
		  	}, shakeRemove);
			}, shakeAdd);
    }
  },
...
Shopify.onCartUpdate = function(cart) {
  ...
  $('body').trigger('refreshCurrency');
  cartUpdateValue = cart;
};

addedModal.$modal.on('show.bs.modal', function () {
  if (addToCartGa4PDP) {
    const quantity = $('input[name="quantity"]').val();
    addToCartGa4PDP({
      quantity,
    });
    sendToGA4Ecommerce('add_to_cart');
  }
});
...
var removeFromCartHandler = function(){
  $('body').on('click', '.header_delete_cartitem_js', function(e){
    e.preventDefault();
    var count_cart = $(this).closest('.tt-cart-list').children().length;
    if (count_cart == 1) {
    	$('.js-tt-cart').find('.active').removeClass('active').find('.tt-dropdown-menu').slideUp(200);
    }
    
    $(this).closest('.tt-item').addClass('removeAfterCompleteAjax');
    const index = $(this).closest('.tt-item').index()
    var variant_id = getIdFromUrl($(this).attr("href"));
    navbarDeleteCart(index);
